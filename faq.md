...
RAI is an ETH backed stablecoin with a [managed float regime](https://en.wikipedia.org/wiki/Managed\_float\_regime). The RAIUSD exchange rate is determined by supply and demand while the protocol that issues RAI tries to stabilize its price by constantly de or revaluing it.
