...
* The amount of system coins `wad` used when calling `buyCollateral`. `wad` must be bigger than zero and bigger than or equal to `uint minBid = minimum(minimumBid`, `subtract(bids[id].amountToRaise`, `bids[id].raisedAmount)).` In case `wad` is bigger than `minBid`, the contract will only request what it needs in order to fill `remainingToRaise`. Note that in order to avoid dusty auctions resulting from computing an `adjustedBid` \(because of precision loss from division\) the contract will automatically request `10**-18` extra system coins
